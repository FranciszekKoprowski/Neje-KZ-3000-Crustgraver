...
impl Engraver {
    pub fn connect(port: &str) -> Result<Self> {
        let state       = Arc::new(Mutex::new(DeviceState { progress: 100, ..Default::default() }));
        ...
        let on_message: MessageCallback = Arc::new(move |msg| {
            match &msg {
                DeviceMessage::CarvingProgress { percent, x, y } => {
                    ...
                    s.progress = *percent;
                    s.position = [*x, *y];
                    if *percent >= 100 { s.burning = false; }
                    cv_cb.notify_all();
                }
    ...
    pub fn burn_image(&self, path: &str, x: u16, y: u16, threshold: u8) -> Result<()> {
        let raw_img   = image::load_image(path)?;
        let thresholded = image::threshold_image(&raw_img, threshold);
    ...
    /// Same as burn_image but accepts a pre-loaded DynamicImage (used by GUI).
    pub fn burn_dynamic_image(
        ...
        let thresholded = image::threshold_image(img, threshold);
        let w = thresholded.width()  as u16;
        let h = thresholded.height() as u16;
