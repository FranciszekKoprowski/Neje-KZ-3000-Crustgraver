...
use engraver::protocol::{MAX_HEIGHT, MAX_WIDTH};

use anyhow::Result;
use eframe::{NativeOptions, egui};
use egui::{
    Color32, ColorImage, Pos2, Rect, RichText, Rounding, Stroke, TextureHandle, TextureOptions,
    Vec2,
};
...
struct App {
    ...
    /// 1-bit thresholded preview, regenerated when threshold or invert changes
    threshold_tex: Option<TextureHandle>,
    /// cache key: (threshold, invert) value used to build threshold_tex
    threshold_tex_at: (u8, bool),
...
impl Default for App {
    fn default() -> Self {
        ...
        Self {
            ...
            preview_tex: None,
            threshold_tex: None,
            threshold_tex_at: (255, false), // sentinel: force rebuild on first frame
...
impl App {
    ...
    fn rebuild_threshold_tex(&mut self, ctx: &egui::Context) {
        if let Some(img) = &self.loaded_image {
            ...
            let thresholded = engraver::image::threshold_image(img, thr, invert);
            let rgba = thresholded.to_rgba8();
            let (w, h) = (rgba.width() as usize, rgba.height() as usize);
            let ci = ColorImage::from_rgba_unmultiplied([w, h], rgba.as_raw());
            self.threshold_tex =
                Some(ctx.load_texture("threshold_preview", ci, TextureOptions::default()));
            self.threshold_tex_at = (thr, invert);
        }
    }
        ...
        if let Some(p) = path {
            ...
            match ::image::open(&p) {
                Ok(img) => {
                    ...
                    let ci = ColorImage::from_rgba_unmultiplied([w, h], rgba.as_raw());
                    self.preview_tex =
                        Some(ctx.load_texture("preview", ci, TextureOptions::default()));
                    self.loaded_image = Some(img);
                    self.image_path = Some(path_str.clone());
                    self.threshold_tex = None;
                    self.threshold_tex_at = (255, false); // sentinel
...
impl eframe::App for App {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        if self
            .engraver
            .as_ref()
            .map(|e| e.lock().unwrap().state().burning)
            .unwrap_or(false)
        {
            ctx.request_repaint_after(std::time::Duration::from_millis(250));
        }

        // rebuild threshold preview if slider moved, invert toggled, or image just loaded
        if self.loaded_image.is_some() && (self.threshold, self.invert) != self.threshold_tex_at {
                            ...
                            ui.horizontal(|ui| {
                                ...
                                match state.temperature {
                                    ...
                                    Some(t) => {
                                        ...
                                        ui.label(
                                            RichText::new(format!("{t} C")).color(col).strong(),
                                        )
                                    }
                                };
                            });
                ...
                egui::Frame::none()
                    ...
                    .show(ui, |ui| {
                        ui.horizontal(|ui| {
                            ...
                            ui.add(
                                egui::Slider::new(&mut self.power, 0..=255).clamp_to_range(true),
                            );
                        });
                        ui.horizontal(|ui| {
                            ui.label("Idle  (0–255):");
                            ui.add(
                                egui::Slider::new(&mut self.idle_power, 0..=255)
                                    .clamp_to_range(true),
                            );
                        });
                        if ui
                ...
                egui::Frame::none()
                    ...
                    .show(ui, |ui| {
                        ui.horizontal(|ui| {
                            ui.label("Origin X:");
                            ui.add(
                                egui::DragValue::new(&mut self.origin_x).clamp_range(0..=MAX_WIDTH),
                            );
                            ui.label("Y:");
                            ui.add(
                        ...
                        ui.horizontal(|ui| {
                            if ui
                                ...
                                .on_hover_text(
                                    "Laser traces the bounding box of the image (low power)",
                                )
                                .clicked()
                            {
                            ...
                            if ui
                                .add(
                                    egui::Button::new(RichText::new("Stop Preview").size(13.0))
                        ...
                        ui.horizontal(|ui| {
                            ...
                            if burn_btn.clicked() {
                                ...
                                let thr = self.threshold;
                                let invert = self.invert;
                                self.with_engraver(|e| {
                                    e.burn_dynamic_image(&img, x, y, thr, invert)
                                });
                            ...
                            if ui
                                .add(
                                    egui::Button::new(RichText::new("Stop").size(13.0))
                                        .fill(Color32::from_rgb(140, 40, 40))
            ...
            .show(ctx, |ui| {
                ...
                egui::Frame::none()
                    ...
                    .show(ui, |ui| {
                        ...
                        ui.horizontal(|ui| {
                            let inv_resp =
                                ui.checkbox(&mut self.invert, RichText::new("Invert").size(12.0));
                            inv_resp.on_hover_text(
                                "Flip black and white before burning.\n\
                                 Use this when your logo is white on a dark background,\n\
                                 or when the result looks like a negative.",
                            );
                            if self.invert {
                                ui.label(
                        ...
                        if let Some(img) = &self.loaded_image {
                            ...
                            if total > 0 {
                                ...
                                let dark = gray.pixels().step_by(4).filter(|p| p[0] < thr).count()
                                    as u64
                                    * 4;
                ...
                egui::Frame::none()
                    ...
                    .show(ui, |ui| {
                        ...
                        // ── D-pad: fixed-size grid so all buttons align perfectly ──
                        let step = self.jog_step;
                        let bsz = Vec2::new(46.0, 34.0);
                        let fill = Color32::from_rgb(44, 44, 58);
                        let fill_home = Color32::from_rgb(38, 60, 90);

                        // row 1: [blank] [UP] [blank]
                        ui.horizontal(|ui| {
                            ui.add_sized(bsz, egui::Label::new("")); // spacer
                            if ui
                                .add_sized(
                                    bsz,
                                    egui::Button::new(RichText::new("UP").size(12.0)).fill(fill),
                                )
                                .clicked()
                            {
                        ...
                        ui.horizontal(|ui| {
                            if ui
                                .add_sized(
                                    bsz,
                                    egui::Button::new(RichText::new("LT").size(12.0)).fill(fill),
                                )
                                .clicked()
                            {
                            ...
                            if ui
                                .add_sized(
                                    bsz,
                                    egui::Button::new(RichText::new("HOME").size(11.0))
                                        .fill(fill_home),
                                )
                                .on_hover_text("Move to centre of work area")
                                .clicked()
                            {
                                let cx = MAX_WIDTH / 2;
                                let cy = MAX_HEIGHT / 2;
                            ...
                            if ui
                                .add_sized(
                                    bsz,
                                    egui::Button::new(RichText::new("RT").size(12.0)).fill(fill),
                                )
                                .clicked()
                            {
                                self.with_engraver(|e| e.jog(step as i32, 0));
                        ...
                        ui.horizontal(|ui| {
                            ui.add_sized(bsz, egui::Label::new(""));
                            if ui
                                .add_sized(
                                    bsz,
                                    egui::Button::new(RichText::new("DN").size(12.0)).fill(fill),
                                )
                                .clicked()
                            {
                                self.with_engraver(|e| e.jog(0, step as i32));
...
fn draw_workarea_map(ui: &egui::Ui, rect: Rect, engraver: &Option<Arc<Mutex<Engraver>>>) {
    ...
    painter.line_segment(
        [Pos2::new(cx.x, rect.top()), Pos2::new(cx.x, rect.bottom())],
        Stroke::new(0.5, dim_gray),
    );
    painter.line_segment(
        [Pos2::new(rect.left(), cx.y), Pos2::new(rect.right(), cx.y)],
        Stroke::new(0.5, dim_gray),
    );
    ...
    if let Some(arc) = engraver {
        ...
        painter.line_segment(
            [Pos2::new(px - ch, py), Pos2::new(px + ch, py)],
            Stroke::new(1.5, orange),
        );
        painter.line_segment(
            [Pos2::new(px, py - ch), Pos2::new(px, py + ch)],
            Stroke::new(1.5, orange),
        );
